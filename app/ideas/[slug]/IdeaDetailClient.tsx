...
import { ArrowLeft, Download, Clock, IndianRupee, TrendingUp, Gauge, Check, Printer, Wand2 } from 'lucide-react'
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" transition={pageTransition} className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <motion.div variants={staggerContainer} initial="initial" animate="animate">
        ...
        <motion.div variants={fadeUpItem} className="flex items-start justify-between gap-4 mb-3">
          <h1 className="text-title">{idea.title}</h1>
          <SaveButton slug={idea.slug} className="mt-1 shrink-0" />
        </motion.div>
      ...
      <div className="grid lg:grid-cols-[280px_1fr] gap-8 items-start">
        <div className="lg:sticky lg:top-28 space-y-3">
          ...
          <Link href={`/builder?idea=${idea.slug}`} className="block roadmap-card w-full text-left bg-yellow hover:-translate-y-0.5 transition-transform">
            <p className="flex items-center gap-1.5 font-bold text-sm"><Wand2 className="w-4 h-4" /> Build your own</p>
            <p className="text-xs text-ink/70 mt-1">Customize a detailed roadmap for your budget & timeline.</p>
          </Link>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => handleDownload(active)} className="btn-primary text-sm px-4 py-2">
                  {justDownloaded === active.id ? <><Check className="w-4 h-4" /> Downloaded</> : <><Download className="w-4 h-4" /> Download PDF</>}
                </button>
              </div>
            </div>
